# -*- coding: utf-8 -*-
# Best VEO3 Bot (VEO-only) — PTB 20.7
# Версия: 2025-09-10 (VEO-only, vertical 9:16 ok, robust send, auto deleteWebhook)

import os, json, time, uuid, asyncio, logging, tempfile
...
from telegram.ext import (
    ...
    CallbackQueryHandler, filters, AIORateLimiter
)
...
# ==========================
#   Public image rehost (KIE Upload only)
# ==========================
...
async def ensure_public_url_from_tg_or_http(ctx: ContextTypes.DEFAULT_TYPE, *, file_id: Optional[str], http_url: Optional[str]) -> Optional[str]:
    """
    Всегда стараемся получить публичный URL, доступный KIE:
    - если прислали фото в TG — качаем байты и грузим на KIE upload;
    - если прислали http(s) — сначала пробуем KIE url-upload, иначе скачиваем и грузим потоково.
    """
    try:
        if file_id:
...
# ==========================
WELCOME = (
    "🎬 *Veo 3* — генерирует видео по тексту или по фото.\n"
    "Поддержка *16:9* и *9:16*. Моды *Fast* и *Quality*.\n"
    "Я сам перезалью ваши фото на публичный хост (KIE Upload).\n\n"
    "Примеры и идеи: {link}\n\n"
    "Выберите режим:".format(link=PROMPTS_CHANNEL_URL)
)
...
async def oai_prompt_master(idea_text: str) -> Optional[str]:
    ...
    system = (
        ...
        "No prefaces, no lists, no brand names. Include lens/optics, camera movement, lighting/palette, "
        "tiny sensory details and subtle audio cues."
    )
    try:
...
def _extract_result_url(data: Dict[str, Any]) -> Optional[str]:
    # 1) приоритет: originUrls (для 9:16) → resultUrls
    ...
    # 3) глубокий поиск по произвольному JSON (исправлено: без точек с запятой и без опечаток)
    def walk(x):
...
def _build_payload_for_veo(prompt: str, aspect: str, image_url: Optional[str], model_key: str) -> Dict[str, Any]:
    payload: Dict[str, Any] = {
        ...
        "enableFallback": aspect == "16:9",  # fallback допустим только для 16:9 (из доки)
    }
    if image_url:
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("mode:"):
        ...
        if mode == "veo_text":
            await query.message.reply_text("VEO: пришлите идею/промпт")
        elif mode == "veo_photo":
    ...
    if data == "card:toggle_photo":
        if s.get("last_image_url"):
            ...
            await query.message.reply_text("Пришлите фото. Я загружу его на публичный хост (KIE Upload).")
        return
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Если это URL картинки — перезальём на KIE и сохраним
    low = text.lower()
...
def _auto_delete_webhook():
    try:
        ...
        log.info("deleteWebhook -> %s %s", r.status_code, r.text)
    except Exception as e:
...
def main():
    ...
    _auto_delete_webhook()  # всегда снимаем вебхук, чтобы polling гарантированно работал
