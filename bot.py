...
# Версия: 2025-09-09 (vertical video fix, KIE-only upload, robust URL extraction + optional chat debug)
...
# OpenAI (опционально для Prompt-Master/Chat)
...
DEBUG_CHAT_PAYLOAD = os.getenv("DEBUG_CHAT_PAYLOAD", "0").strip() == "1"
...
def _kie_upload_bytes(data: bytes, filename: str, mime: str = "image/jpeg") -> Optional[str]:
    """Загрузка бинаря в KIE upload. Возвращаем прямую download/fileUrl."""
    ...
    try:
        ...
        log.warning("KIE stream upload bad response: %s", j)
    except Exception as e:
        log.exception("KIE stream upload failed: %s | body=%s", e, getattr(r, "text", "")[:500])
    return None

def _kie_upload_from_url(file_url: str, filename: Optional[str] = None) -> Optional[str]:
    """Прокачиваем чужой URL через KIE (сервер сам скачает)."""
    ...
    try:
        ...
        if j.get("success") and j.get("code") == 200:
            d = j.get("data") or {}
            return (d.get("downloadUrl") or d.get("fileUrl") or "").strip() or None
        log.warning("KIE url upload bad response: %s", j)
    except Exception as e:
        log.exception("KIE url upload failed: %s | body=%s", e, getattr(r, "text", "")[:500])
    return None
...
async def ensure_public_url_from_tg_or_http(ctx: ContextTypes.DEFAULT_TYPE, *, file_id: Optional[str], http_url: Optional[str]) -> Optional[str]:
    """Берём фотку из TG или публичного URL → получаем безопасную публичную ссылку через KIE Upload."""
    try:
        if file_id:
            ...
            if not ext:  # Telegram иногда не даёт расширение
                name += ".jpg"
            return _kie_upload_bytes(raw, name, mime=mime)

        if http_url:
            u_name = os.path.basename(http_url.split("?")[0]) or "image.jpg"
            # сначала пробуем "URL upload"
            url_up = _kie_upload_from_url(http_url, filename=u_name)
            if url_up:
                return url_up
            # если не получилось — скачиваем и грузим потоково
...
def _coerce_url_list(value) -> List[str]:
    """Универсальная нормализация массивов ссылок (строка/массив/объект/JSON-строка)."""
    urls: List[str] = []
...
def _extract_result_url(data: Dict[str, Any]) -> Optional[str]:
    """
    KIE для не-16:9 возвращает originUrls (оригинальный размер),
    для 16:9 — resultUrls. Берём originUrls в приоритете.
    """
    ...
    # 3) fallback: глубокий поиск http*.mp4/mov/webm
    def walk(x):
...
def _build_payload_for_veo(prompt: str, aspect: str, image_url: Optional[str], model_key: str) -> Dict[str, Any]:
    """Формируем payload строго по доке: aspectRatio ∈ {'16:9','9:16'}, enableFallback доступен только для 16:9."""
...
def submit_kie_veo(prompt: str, aspect: str, image_url: Optional[str], model_key: str) -> Tuple[bool, Optional[str], str, Dict[str, Any]]:
    ...
    if status == 200 and code == 200:
        ...
        if task_id:
            return True, task_id, "Задача создана.", payload
        return False, None, "Ответ KIE без taskId.", payload
    return False, None, _kie_error_message(status, j), payload

def get_kie_veo_status(task_id: str) -> Tuple[bool, Optional[int], Optional[str], Optional[str], Dict[str, Any]]:
    ...
    if status == 200 and code == 200:
        ...
        return True, flag, msg, _extract_result_url(data), data
    return False, None, _kie_error_message(status, j), None, (j or {})
...
async def poll_veo_and_send(chat_id: int, task_id: str, gen_id: str, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        while True:
            ...
            ok, flag, msg, res_url, data = await asyncio.to_thread(get_kie_veo_status, task_id)
            ...
            if flag == 1:
                if DEBUG_CHAT_PAYLOAD:
                    try:
                        txt = json.dumps(data, ensure_ascii=False, indent=2)
                        await ctx.bot.send_message(chat_id, f"🧾 KIE статус (flag=1):\n```json\n{txt[:3500]}\n```", parse_mode=ParseMode.MARKDOWN)
                    except Exception:
                        pass
...
async def health(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    parts = [
        ...
        f"DEBUG_CHAT_PAYLOAD: `{int(DEBUG_CHAT_PAYLOAD)}`",
    ]
...
async def show_card_veo(update: Update, ctx: ContextTypes.DEFAULT_TYPE, edit_only_markup: bool = False):
    ...
    try:
        ...
        try:
            ...
            log.exception("show_card_veo fallback send failed: %s", e2)
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "card:toggle_photo":
        if s.get("last_image_url"):
            ...
            await query.message.reply_text("Пришлите фото (или публичный URL). Я загружу в KIE Upload.")
        return
    ...
    if data == "card:generate":
        ...
        ok, task_id, msg, sent_payload = await asyncio.to_thread(
            submit_kie_veo, s["last_prompt"].strip(), s.get("aspect", "16:9"),
            s.get("last_image_url"), s.get("model", "veo3_fast")
        )

        if DEBUG_CHAT_PAYLOAD:
            pretty_payload = json.dumps(sent_payload, ensure_ascii=False, indent=2)
            await query.message.reply_text(f"📦 Payload в KIE:\n```json\n{pretty_payload}\n```", parse_mode=ParseMode.MARKDOWN)
...
if __name__ == "__main__":
    # если когда-то был webhook — снимите вручную:
