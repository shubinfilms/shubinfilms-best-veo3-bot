...
# Версия: 2025-09-13 (полная сборка с фиксом Banana, ценами и возвратами)
...
if not REDIS_URL:
    raise RuntimeError("REDIS_URL is not set (для постоянного баланса)")
...
# Prompt-Master (OpenAI — опционально)
...
DEFAULT_STATE = {
    ...
    "mj_wait_ts": 0.0,           # анти-спам «Рисую…» ≥ 40 сек
    # Banana
    "banana_mode": False,
    "banana_photos": [],
    "banana_prompt": None,
...
async def oai_prompt_master(idea_text: str) -> Optional[str]:
    """
    Возвращает структурированный кинопромпт.
    Если есть указание «озвучка на русском», диалог остаётся по-русски,
    Lip-sync — для русского, Audio — русское VO.
    """
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "mode:banana":
        ...
        await q.message.reply_text(
            ...
            "Затем *напишите*, что нужно сделать: заменить одежду, убрать/добавить макияж, поменять локацию, добавить людей, объединить фото.\n"
    ...
    if data == "mode:chat":
        s["mode"]="chat"
        await q.message.reply_text("💬 Обычный чат включён. Напишите вопрос."); return
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Обычный чат
    if s.get("mode") == "chat":
        # тут можешь подставить свой LLM; пока — простой эхо
        await update.message.reply_text(text); return

    # По умолчанию — VEO: кладём промпт в карточку
    s["last_prompt"] = text
...
async def on_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if s.get("banana_mode") and s.get("mode") == "banana":
        ...
        await update.message.reply_text(
            ...
            "Теперь *напишите*, что нужно сделать с изображениями: объединить людей, поменять одежду/макияж, сменить локацию, добавить фон и т.п.",
            parse_mode=ParseMode.MARKDOWN, reply_markup=banana_kb()
        ); return

    # Иначе — референс для VEO
