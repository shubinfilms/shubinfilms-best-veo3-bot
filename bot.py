...
try:
    ...
    openai = None  # Prompt-Master/чат отключатся, если ключа нет
...
def build_card_text(s: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"• Режим: <b>{esc(model)}</b>",
        f"• Промпт: <b>{has_prompt}</b>",
        f"• Референс: <b>{has_ref}</b>",
    ]
    return "\n".join(lines)
    ...
    for k in ("successFlag", "status", "state"):
        if k in data:
            ...
            try:
                ...
                elif s in ("failed", "error", "3", "2"): flag = 3
                else: flag = 0
                break
...
def _kie_error_message(status_code: int, j: Dict[str, Any]) -> str:
    ...
    if code in (401, 403):
        ...
        base = "Запрос отклонён модерацией / включите enableFallback (422)."
    else:
...
async def health(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    txt = "<b>Health</b>\n" + esc(json.dumps(info, ensure_ascii=False, indent=2))
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "faq":
        await query.message.reply_text(
            "FAQ:\n"
            "• Рендер: Veo 3 Fast / Quality (если доступно).\n"
            "• Формат: 16:9 / 9:16 (вертикаль).\n"
            "• Видео придёт сюда. Если URL не стримится — пришлю файл.\n"
    ...
    if data == "card:generate":
        ...
        await query.message.reply_text(
            f"🚀 Задача отправлена ({'Fast' if s.get('model')=='veo3_fast' else 'Quality'}). taskId={task_id}"
        )
