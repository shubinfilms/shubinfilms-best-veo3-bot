...
class NoRetryService(SunoService):
    def __init__(self, responses: list[dict]) -> None:
...
def test_vocal_similarity_ok_no_retry() -> None:
    service = NoRetryService([{"data": {"lyrics": "hello world"}}])
    meta = TelegramMeta(
        chat_id=1,
        msg_id=10,
        title="Demo",
    ...
    result = service._process_strict_delivery(
        task=SunoTask(task_id="task-1", callback_type="complete", items=[]),
        meta=meta,
        link=None,
    ...
    assert result.get("similarity", 0.0) >= 0.75
